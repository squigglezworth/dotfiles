...
for i in disk-{1,2,3} usb; do sudo mount /mnt/$i; done
echoDone
...
ln -fs $PWD/rofi/ $HOME/.config/; echoDone
...
echo -e "\t Configuring music stuff:"

echo -ne "\t\t Linking music dir ... "
ln -fs /mnt/disk-1/media/music/ $HOME/; echoDone
...
sudo systemctl disable --now mpd mpd.socket &>>/tmp/dotfiles.log
